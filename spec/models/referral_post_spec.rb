...
RSpec.describe ReferralPost, type: :model do
  ...
  it "is valid with required attributes" do
    post = ReferralPost.new(user: user, company_verification: company_verification, company_name: "Example", title: "Job")
    expect(post).to be_valid
  end

  it "validates presence of title" do
    post = ReferralPost.new(user: user, company_verification: company_verification, company_name: "Example")
    expect(post).not_to be_valid
    expect(post.errors[:title]).to be_present
  end

  it 'scopes active posts' do
    ReferralPost.create!(user: user, company_verification: company_verification, company_name: 'Example', title: 'A', status: :active)
    ReferralPost.create!(user: user, company_verification: company_verification, company_name: 'Example', title: 'B', status: :closed)
